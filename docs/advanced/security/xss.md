...
根据攻击的来源，XSS 攻击可分为存储型、反射型、DOM 型三种。
...
**方案一**：在用户提交时，由前端过滤输入，然后提交到后端。

此方案不可行。一旦攻击者绕过前端过滤，直接构造请求，就可以提交恶意代码了。

**方案二**：后端在写入数据库前，对输入进行过滤，然后把安全的内容，返回给前端。

此方案不可行。在提交阶段，我们并不确定内容要输出到哪里。如果作为 HTML 拼接页面时，需要使用过滤/转译后的字符串；如果作为变量值在 JavaScript 脚本中执行时，需要使用转译前的字符串，比如：用于 Vue 等模板的展示、内容长度的计算等。
...
在 [Unleashing an Ultimate XSS Polyglot](https://github.com/0xsobky/HackVault/wiki/Unleashing-an-Ultimate-XSS-Polyglot) 一文中，提到有以下字符串能够检测到存在于 HTML 属性、HTML 文字内容、HTML 注释、跳转链接、内联 JavaScript 字符串、内联 CSS 样式表等多种上下文中的 XSS 漏洞，也能检测 `eval()`、`setTimeout()`、`setInterval()`、Function()、innerHTML、document.write() 等 DOM 型 XSS 漏洞，并且能绕过一些 XSS 过滤器。
